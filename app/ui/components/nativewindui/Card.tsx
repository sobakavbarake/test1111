...
import { Text, TextClassContext } from "@/app/ui/components/nativewindui/Text"
...
>(({ className, rootClassName, ...props }, ref) => (
  <View className={cn("bg-card", rootClassName)}>
    <View
      ref={ref}
  ...
  if (linearGradientColors) {
    return (
      <LinearGradient colors={linearGradientColors ?? []} className="pt-4">
        ...
        <View ref={ref} className={cn("px-5 space-y-1.5 pb-4", className)} {...props} />
      </LinearGradient>
    )
  }
  return (
    <>
      ...
      <View ref={ref} className={cn("px-5 space-y-1.5 py-4", className)} {...props} />
    </>
  )
})
...
function CardTitle({ className, ...props }: React.ComponentPropsWithoutRef<typeof Text>) {
  return (
    <Text
      role="heading"
      aria-level={3}
      className={cn(" ios:font-bold text-3xl font-medium leading-none tracking-tight", className)}
      {...props}
    />
  )
}
...
function CardDescription({ className, ...props }: React.ComponentPropsWithoutRef<typeof Text>) {
  return <Text className={cn("text-base-content leading-5", className)} {...props} />
}
